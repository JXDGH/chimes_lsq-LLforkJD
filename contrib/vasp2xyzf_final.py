...
for LINE in IFSTREAM:
	...
	if "in kB" in LINE:
        ...
        if "free  energy   TOTEN" in LINE: 
            ...
            TEMP_ENER = LINE.split()[4]
    	    if "***" in TEMP_ENER:
    		    TEMP_ENER = "10000000.0"
    	    else:
    		    TEMP_ENER = float(TEMP_ENER)*23.06 # Convert from eV (VASP) to kcal/mol (house_md)

            if frame_buffer:
                OFSTREAM.write(str(TOTAL_ATOMS) + '\n')
                OFSTREAM.write(frame_buffer.pop(0))
               
                if INCL_ENERGY:
                    OFSTREAM.write(" " + str(TEMP_ENER))
                OFSTREAM.write('\n')
                for atom_line in frame_buffer:
                    OFSTREAM.write(atom_line + '\n')
                frame_buffer = [] # Clear the buffer for the next frame

        # Read atom positions are start buffering the frame data
        if "POSITION" in LINE:
            TOTAL_FRAMES += 1
            if TOTAL_FRAMES%SKIP == 0: 
                ...
                frame_header = "NON_ORTHO {} {} {}".format(BOXLENS[0], BOXLENS[1], BOXLENS[2].rstrip(')'))
                if INCL_STRESS:
                    frame_header += " {} {} {}".format(TENSOR_XX, TENSOR_YY, TENSOR_ZZ)
                elif INCL_ALLSTR:
                    frame_header += " {} {} {} {} {} {}".format(TENSOR_XX, TENSOR_YY, TENSOR_ZZ, TENSOR_XY, TENSOR_ZX, TENSOR_YZ)
                frame_buffer.append(frame_header)
        ...
        if START_READ == 1: # Skip the extra line following "POSITIONS"
            START_READ = 2
            continue
	...
	if START_READ == 2 and READ_ATOMS < TOTAL_ATOMS:
		...
		frame_buffer.append(ATOM_TYPES[READ_ATOMS] + " " + " ".join(LINE))
		READ_ATOMS += 1
